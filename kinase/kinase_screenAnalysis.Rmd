...
KINASE_OUT=paste(OUT,'/Table.S1.4.KinaseScreen_Hits.txt',sep='')
```
...
for (ki in as.character(unique(data$KI))){
  ...
  for (conc in concs){
      ...
      for_conf=qnorm(0.95)*sd(gem_proportions)/sqrt(N)
      rangesOverlap=FALSE
      ...
      #Also add the expected survival from Bliss
  }
}
```
...
fit_weibull_separately=function(data,ki){
 tryCatch(
   { 
     ...
     #https://www.google.com/url?sa=t&rct=j&q=&esrc=s&source=web&cd=8&ved=0CEAQFjAHahUKEwihobCdnZrJAhUKaT4KHYsmB3A&url=http%3A%2F%2Fwww.jstatsoft.org%2Farticle%2Fview%2Fv012i05%2Fv12i05.pdf&usg=AFQjCNG8wZJ4XniU3Zfj95e0GqOk2lm_tg&sig2=F1D2aW2OcB9AzSFGHOvPcw
...
pdf(paste(OUT,'/KinaseInhibitorScreen_Ratios.pdf',sep=''),width=5,height=5)
par(mar=c(6,6,4,2)+1)
...
write.table(hits,file=KINASE_OUT,sep='\t',quote=F,row.names=F,col.names=T)
```
